...
export function loadSprites(base = 'assets/sprites') {
  ...
  loading = fetch(`${base}/atlas.json`)
    ...
    .then((json) => {
      atlas = json;
      return Promise.all(Object.entries(json.classes).map(([classId, meta]) =>
        new Promise((resolve) => {
          const img = new Image();
          img.onload = () => { SHEETS.set(classId, { img, meta }); resolve(true); };
          // A sheet that will not load is not an error worth stopping for —
          // that class simply keeps the old shape.
